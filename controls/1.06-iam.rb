...
control "cis-gcp-#{control_id}-#{control_abbrev}" do
  ...
  title "[#{control_abbrev.upcase}] Ensure that IAM users are not assigned Service Account User
  or Service Account Token Creator roles at project level"

  desc "It is recommended to assign the Service Account User (iam.serviceAccountUser) and Service Account Token Creator
  (iam.serviceAccountTokenCreator) roles to a user for a specific service account rather than assigning the role to a user at project level."
  
  desc 'rationale', "A service account is a special Google account that belongs to an application or a virtual 
  ...
  iam.serviceAccountTokenCreator roles to a user for a project gives the user access to all 
  service accounts in the project, including service accounts that may be created in the 
  ...
  ['roles/iam.serviceAccountUser', 'roles/iam.serviceAccountTokenCreator'].each do |role|
    describe "[#{gcp_project_id}] A project-level binding of ServiceAccountUser or ServiceAccountTokenCreator" do
      ...
      it { expect(iam_bindings_cache.iam_bindings[role]).to eq nil }
      # it { should eq nil }
    end
  end
end
